...
<!--          <img src="../../static/images/right.svg" alt="">-->
          <div class="success-tips">
              <p class="tips1">您已成功购买 {{result.course_data_list.length}} 门课程！</p>
              <p class="tips2">你还可以加入QQ群 <span>747556033</span> 学习交流</p>
          </div>
        </div>
        <div class="order-info">
            <p class="info1"><b>付款时间：</b><span>{{result.pay_time.replace("T"," ")}}</span></p>
            <p class="info2"><b>付款金额：</b><span >{{result.real_price}}</span></p>
            <p class="info3"><b>课程信息：</b><span><span v-for="course in result.course_data_list"><router-link :to="`/courses/${course.course_id}`">《{{course.course_name}}》</router-link></span></span></p>
        </div>
  ...
  export default{
    ...
    created(){
      ...
      this.$axios.get(`${this.$settings.Host}/payments/alipay/result/`+location.search).then(response=>{
         ...
         this.$message.error("无法获取支付结果！支付失败或者参数无效！")
      });
    },
    components:{
      Header,
